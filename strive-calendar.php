    ...
    class Strive_Content_Calendar
    {
        ...
        public static function instance()
        {
            if (!isset(self::$__instance) && !(self::$__instance instanceof self)) {
                ...
                if (scc_freemius()->can_use_premium_code()) {
        ...
        public function __construct()
        {
            ...
            // Add message for expired trials
            scc_freemius()->add_action('connect/before', [$this, 'trial_expired_message']);
            // Always enqueue my styles on the license activation page
            add_action('admin_enqueue_scripts', [$this, 'license_activation_styles']);
        }

        public function trial_expired_message()
        {
            if (scc_freemius()->is_trial_utilized()) {
                echo '<div class="license-expired-notice">';
                echo '<h1>' . esc_html('Your license has expired', 'strive') . '</h1>';
                echo '<p>' . esc_html('Thanks so much for trying Strive, we hope you loved it!') . '</p>';
                echo '<p>' . __('<b>Your 30-day free trial has expired</b>. You can activate a subscription to continue using Strive on your site.') . '</p>';
                echo '<a class="purchase button-primary" href="' . scc_freemius()->checkout_url() . '">Purchase a subscription</a>';
                echo '<p>' . __('If you have any questions before your purchase or if you think there\'s been an error, you can <a href="https://strivecalendar.com/contact/" target="_blank">contact us here</a> 
                    or <a href="https://strivecalendar.com/account/" target="_blank">login to your account</a> to review your license details.') . '</p>';
                echo '</div>';
            }
        }
        ...
        public function settings_page_markup()
        {
            ...
            <div id="strive-content-calendar-parent" class="<?php echo esc_attr($classes); ?>">
                <div class="branding-bar">
                    <div class="logo"><img src="<?php echo STRIVE_CC_URL . 'img/strive-icon.svg' ?>" /></div>
                    <div class="title">Strive Content Calendar</div>
                    <?php // Upgrade ad for free users
                    if (scc_freemius()->is_not_paying()) :
                        $now = new DateTime();
                        $expires = new DateTime(scc_freemius()->get_site()->trial_ends);
                        $diff = $now->diff($expires);
                        $time_left = $diff->format('%a');
                        if ($time_left == 0) {
                            $time_left = $diff->format('%h') . ' ' . esc_html__('hours', 'strive');
                        } else {
                            $time_left .= ' ' . esc_html__('days', 'strive');
                        } ?>
                        <div class="upgrade-prompt">
                            <p>Your free trial ends in <b><?php esc_html_e($time_left); ?></b>. <a href="<?php echo esc_url(scc_freemius()->checkout_url()); ?>">Upgrade now</a> to avoid disrupting your workflow.</p>
                        </div>
                    <?php endif; ?>
                    <div class="knowledgebase"><a href="https://strivecalendar.com/knowledgebase/" target="_blank">Knowledge Base <span class="dashicons dashicons-external"></span></a></div>
                </div>
                <nav class="nav-tab-wrapper">
                    ...
                    <?php if (self::permission_manager('admin')) : ?>
                        ...
                        <?php
                        // Add upgrade message for free trial users
                                                                                                                                                                                    if (scc_freemius()->is_not_paying()) : ?>
                            <a href="<?php echo esc_url(scc_freemius()->checkout_url()); ?>" class="nav-tab <?php if ($tab === 'upgrade'): ?>nav-tab-active<?php endif; ?>">
                                <?php esc_html_e('Upgrade Now', 'strive'); ?>
                            </a>
                        <?php endif; ?>
                        <a href="?page=strive-content-calendar&tab=account" class="nav-tab account <?php if ($tab === 'account'): ?>nav-tab-active<?php endif; ?>">
                            <?php esc_html_e('Account', 'strive'); ?>
                        </a>
                        <a href="?page=strive-content-calendar&tab=contact" class="nav-tab contact <?php if ($tab === 'contact'): ?>nav-tab-active<?php endif; ?>">
                            <?php esc_html_e('Contact', 'strive'); ?>
                        </a>
                        <a href="?page=strive-content-calendar&tab=affiliation" class="nav-tab affiliation <?php if ($tab === 'affiliation'): ?>nav-tab-active<?php endif; ?>">
                            <?php esc_html_e('Affiliate Program', 'strive'); ?>
                        </a>
                    <?php endif; ?>
                    <?php
